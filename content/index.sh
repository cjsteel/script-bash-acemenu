...
#
SCRIPT_DIR=~/bin/acemenu/content
#echo ${SCRIPT_DIR}
...
# Generate parent menu (index.md) and menu items
#
MENUITEMS=""
while IFS=$'
' read -r line || [[ -n "$line" ]]; do
   echo "* [${line}](${line}/${line}.md)" >> ${MARKDOWN_INDEX}
   # create child directories description   
   # generate index.html for all child directories
   pandoc --template ${SCRIPT_RESOURCES_DIRECTORY}/default.html5 ${line}/${line}.md --from markdown --to html -o ${line}/${line}.html
   # insert our css reference  after 7th line preceeded by two (escaped) spaces
   sed -i '7 a\ \ <link rel="stylesheet" href="../.resources/css/styles.css">' ${line}/${line}.html
   # replace .md with .html
   sed -i 's/.md/.html/g' ${line}/${line}.html
